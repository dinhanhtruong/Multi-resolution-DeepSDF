...
import pytorch_lightning.utilities as plutil
...
from shapeformer.common import *
...
class VQConvONet(pl.LightningModule):
    def __init__(self, Xct_as_Xbd=False, encoder_opt=None, decoder_opt=None, quantizer_opt=None, vq_beta=1., \
        ...
        self.encoder  = self.encoder_opt
        self.decoder  = self.decoder_opt
        if quantizer_opt is not None:
            self.quantizer = self.quantizer_opt
    ...
    def training_step(self, batch, batch_idx, stage="train"):
        ...
        self.log(f'codes', self.quantizer.number_of_codes_used)
        return losses['loss']
